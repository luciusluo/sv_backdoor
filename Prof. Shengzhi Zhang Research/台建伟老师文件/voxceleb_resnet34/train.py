...
import dataset as D
from backbone import VGGVox1, OnlineTripletLoss, ResNet34_Vox1
...
net = ResNet34_Vox1(num_classes=1211, emb_dim=c.embedding_dim)
...
def evaluation():
    ...
    test_pairs_path = os.path.join(c.VoxCeleb1_Dir, 'list_test_hard.txt')
...
def pretrain():
    ...
    for epoch in range(1, c.pretrain_epoch_num + 1):
        ...
        # ------------------------------------------------------------------------------------------------ #
        if epoch > 0:
            torch.save(net.state_dict(), c.save_path + '/pretrain/epoch_new')
...
if __name__ == '__main__':
    ...
    pertrain_dict = torch.load(c.save_path + '/pretrain/epoch_33')
    net.load_state_dict(pertrain_dict)	
    result = evaluation()
    print('test set:'
                    '\n EER:%.2f  minDCF:%.3f' % (result[0], result[1]))
    #'''
