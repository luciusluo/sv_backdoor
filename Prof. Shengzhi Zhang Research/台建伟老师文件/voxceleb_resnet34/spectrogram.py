...
def save_feature(save_path=None):
	...
	if len(os.listdir(save_path)) == 0:
		...
		for speaker in tqdm(speakers):
			...
			for speaker_utt in np.sort(os.listdir(os.path.join(path, speaker))):


				filename_path = os.path.join(os.path.join(path, speaker), speaker_utt)
				if not os.path.exists(os.path.join(os.path.join(save_path, speaker), speaker_utt)):
					os.makedirs(os.path.join(os.path.join(save_path, speaker), speaker_utt))

				for wav_name in np.sort(os.listdir(filename_path)):
				# Read wav file and extract feature
					wav_path = os.path.join(path, speaker, speaker_utt, wav_name)
					spec = get_spectrum(wav_path)
					feature_path = os.path.join(save_path, speaker, speaker_utt, wav_name)
